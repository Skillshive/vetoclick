...
export default function Create({}: CreateProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.post(route('pets.store'), formData as any, {
      onSuccess: () => {
        showToast({
          type: 'success',
      ...
      onError: (errors: any) => {
        ...
        showToast({
          type: 'error',
          message: t('common.pet_create_error') || 'Error creating pet',
          duration: 3000,
        });
      },
      onFinish: () => {
...
console.log('breedOptions',breedOptions);
  ...
  const getImageUrl = () => {
    if (data.profile_img instanceof File) {
      return URL.createObjectURL(data.profile_img);
    }
    return "/assets/default/pet-placeholder.jpg";
  };

  const breadcrumbs: BreadcrumbItem[] = [
    { title: t('common.pets') || 'Pets', path: route('pets.index') },
    { title: t('common.add_pet') || 'Add Pet' },
  ];
            ...
            <div className="flex items-center gap-1">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {t('common.add_pet_description') || 'Add a new pet to your account'}
                </p>
              </div>
            </div>
          </div>
