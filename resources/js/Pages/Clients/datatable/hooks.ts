...
import { Client, TableSettings } from "./types";
import { router } from "@inertiajs/react";

declare const route: (name: string, params?: any, absolute?: boolean) => string;

interface UseClientDatatableProps {
  initialData: {
    data: {
      data: Client[];
    };
...
export function useClientTable({ initialData, initialFilters }: UseClientDatatableProps) {
  // Data state
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-clients",
    {},
  );

  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-clients",
    {},
  );
  ...
  const tableMeta = {
    deleteRow: (row: any) => {
      ...
      setClients(prevClients => ({
        ...
        data: {
          ...prevClients.data,
          data: prevClients.data.data.filter((client: Client) => client.uuid !== row.original.uuid)
        }
      }));

      router.delete(route('clients.destroy', row.original.uuid), {
        preserveState: true, 
        preserveScroll: true,
        onSuccess: () => {
          setTimeout(() => {
    ...
    deleteRows: async (rows: any[]) => {
      ...
      setClients(prevClients => ({
        ...
        data: {
          ...prevClients.data,
          data: prevClients.data.data.filter((client: Client) => !rowIds.includes(client.uuid))
        }
      }));
