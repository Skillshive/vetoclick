...
import type { OrderDatatableProps } from "./types";
...
export default function OrderDatatable({
  ...
  const {
    ...
    initialData: ordersResource.data,
    initialFilters: filters,
  });

  useEffect(() => {
    setOrders({ data: ordersResource.data });
  }, [ordersResource.data, setOrders]);
  ...
  const handleBulkDeleteRows = () => {
    ...
    setTimeout(async () => {
      try {
        ...
        showToast({
          type: "success",
          message: t("common.orders_deleted_success", { count: deleteCount }),
        });
      } catch (error) {
        console.error(error);
        setConfirmBulkDeleteLoading(false);
  ...
  return (
    <>
            ...
            >
                ...
                >
                  ...
                  <TBody>
                    {table.getCoreRowModel().rows.length > 0 ? (
                      ...
                      <Tr>
                        <Td colSpan={table.getVisibleLeafColumns().length} className="py-8 text-center">
                ...
                <div className="pointer-events-none sticky inset-x-0 bottom-0 z-5 flex items-center justify-end">
                  <div className="w-full max-w-xl px-2 py-4 sm:absolute sm:-translate-y-1/2 sm:px-4">
                    <div className="dark:bg-dark-50 dark:text-dark-900 pointer-events-auto flex items-center justify-between rounded-lg bg-gray-800 px-3 py-2 font-medium text-gray-100 sm:px-4 sm:py-3">
                      <p>
                        <span>
                          {table.getSelectedRowModel().rows.length} {t("common.selected")}
                        </span>
              ...
              {table.getCoreRowModel().rows.length && (
                ...
                >
                  <PaginationSection
                    table={table}
                    totalRows={ordersResource.meta?.total ?? table.getRowCount()}
                  />
                </div>
              )}
            </Card>
      ...
      <ConfirmModal
        ...
        messages={{
          pending: {
            description: t("common.confirm_delete_orders", {
        ...
        state={bulkDeleteError ? "error" : bulkDeleteSuccess ? "success" : "pending"}
      />
    </>
  );
}
