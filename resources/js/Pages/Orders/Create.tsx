...
const CreateOrder: React.FC<CreateOrderProps> = ({ suppliers, products: initialProducts }) => {
    const { showToast } = useToast();
    const { t } = useTranslation();
    const [validationErrors, setValidationErrors] = useState<Partial<Record<keyof OrderFormValues, string>>>({});
    const [products] = useState<Product[]>(initialProducts || []);

    const { data, setData, post, processing, errors, reset } = useForm<OrderFormValues>({
        reference: '',
        supplier_id: '',
        order_type: '',
        subtotal: '0',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('orders.store'), {
            onSuccess: () => {
                showToast({
                    type: 'success',
                    message: t('common.order_created_success') || 'Order created successfully',
                    duration: 3000,
                });
                reset();
                setValidationErrors({});
            ...
            onError: (errors: Record<string, string>) => {
                ...
                showToast({
                    type: 'error',
                    message: t('common.order_create_error') || 'Failed to create order',
                    duration: 3000,
                });
            }
        });
    ...
    return (
       <MainLayout>
             <Page 
               title={t("common.metadata_titles.orders_create") || "New Order"}
               description={t("common.page_descriptions.orders_create") || "Create a new order by selecting products and quantities."}
             >
                   ...
                   <div className="flex items-center gap-1">
                     <div>
                       ...
                       <p className="text-sm text-gray-500 dark:text-gray-400">
                         {t('common.new_order_description') || 'Create a new order'}
                       </p>
                     </div>
                   </div>
                 </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex items-center justify-end space-x-3 pt-4">
                            ...
                            <Button
                                ...
                                color="primary"
                            >
                                {processing ? t('common.submitting') : t('common.create_order') || 'Create Order'}
                            </Button>
                        </div>
                    </Card>
                </div>
