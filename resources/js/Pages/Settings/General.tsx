...
export default function General({ user, isVeterinarian, veterinaryInfo, phoneVerified: initialPhoneVerified }: ProfilePageProps) {
  ...
  const { data, setData, post, processing, errors, reset } = useForm({
    ...
    accepts_video_calls: veterinaryInfo?.accepts_video_calls || false,
    });
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 [&_.prefix]:pointer-events-none">
                      ...
                      <div className="input-root">
                        <label className="input-label block mb-1.5">
                          <span className="input-label">{t('common.accepts_video_calls') || 'Accepts Video Calls'}</span>
                        </label>
                        <div className="mt-1.5">
                          <Switch
                            checked={data?.accepts_video_calls || false}
                            onChange={(e) => {
                              setData('accepts_video_calls', e.target.checked);
                            }}
                            color="primary"
                            variant="basic"
                          />
