...
const RecentConsultations = ({ t }: { t: TranslateFn }) => (
    <div className="bg-white rounded-lg border shadow-sm">
        <div className="flex justify-between items-center p-4 border-b">
            ...
            <button className="flex items-center gap-1.5 text-sm bg-primary-600 text-white py-1.5 px-3 rounded-md hover:bg-primary-700 transition">
                <Plus size={16} /> {t('common.vet_dashboard.pet_modal.new_entry')}
            </button>
        </div>
        <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
                <tr>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{t('common.vet_dashboard.pet_modal.sample.consultation_reason')}</td>
...
const Vaccinations = ({ t }: { t: TranslateFn }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg border shadow-sm">
            ...
            <ul className="divide-y divide-gray-200 p-4">
                 <li className="flex justify-between items-center py-2">
                    <div>
                        <p className="text-sm font-medium text-gray-900">{t('common.vet_dashboard.pet_modal.sample.dhpp')}</p>
                        <p className="text-xs text-gray-500">{t('common.vet_dashboard.pet_modal.expires_on', { date: '12/17/2024' })}</p>
                    </div>
                ...
                {/* ... other list items ... */}
                 <li className="flex justify-between items-center py-2">
                    <div>
                        <p className="text-sm font-medium text-gray-900">{t('common.vet_dashboard.pet_modal.sample.bordetella')}</p>
                        <p className="text-xs text-gray-500">{t('common.vet_dashboard.pet_modal.expires_on', { date: '06/09/2024' })}</p>
                    </div>
        ...
        <div className="bg-white rounded-lg border shadow-sm">
            <div className="flex justify-between items-center p-4 border-b">
                <h4 className="text-lg font-semibold text-gray-800">{t('common.vet_dashboard.pet_modal.reminders')}</h4>
                <button className="p-1 text-gray-400 hover:text-gray-700"><Plus size={18} /></button>
            </div>
            <ul className="divide-y divide-gray-200 p-4">
                <li className="flex justify-between items-center py-2">
                    <p className="text-sm font-medium text-gray-900">{t('common.vet_dashboard.pet_modal.sample.proheart')}</p>
    ...
    <div className="space-y-4">
        <div className="bg-white p-4 rounded-lg border shadow-sm">
            <div className="flex justify-between items-center text-sm border-b pb-2 mb-2">
                ...
                <p className="text-gray-500">09/20/2023 - {t('common.vet_dashboard.pet_modal.sample.wellness_visit')}</p>
            </div>
            <p className="text-sm text-gray-700">
                {t('common.vet_dashboard.pet_modal.sample.note_body')}
            </p>
        </div>
        {/* ... other notes ... */}
    </div>
        ...
        <div className="client-info-item">
            ...
            <span>{t('common.vet_dashboard.pet_modal.sample.address')}</span>
                            ...
                            <div className="p-6 bg-gray-50/70 dark:bg-dark-600/50">
                                {activeTab === 'consultations' && <RecentConsultations t={t} />}
                                {activeTab === 'vaccinations' && <Vaccinations t={t} />}
                                {activeTab === 'notes' && <Notes t={t} />}
                            </div>
                        </div>
                    </DialogPanel>
                </TransitionChild>
