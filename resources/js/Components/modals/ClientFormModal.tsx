...
import { Client } from '@/Pages/Clients/datatable/types';
...
export default function ClientFormModal({ isOpen, onClose, client, errors }: ClientFormModalProps) {
    const { showToast } = useToast();
    const isEditing = !!client;
    const { t } = useTranslation();
    const [validationErrors, setValidationErrors] = useState<Partial<Record<keyof ClientFormData, string>>>({});

    const { data, setData, post, put, processing, reset } = useForm<ClientFormData>({
        ...
        if (isEditing) {
            put(route('clients.update', client.uuid), {
                onSuccess: () => {
                    showToast({
                        type: 'success',
                        message: t('common.client_updated_success') || 'Client updated successfully',
                        duration: 3000,
                    });
                    setValidationErrors({});
                    onClose();
                ...
                onError: (errors: any) => {
                    showToast({
                        type: 'error',
                        message: t('common.client_update_error') || 'Failed to update client',
                        duration: 3000,
                    });
                }
            });
            ...
            post(route('clients.store'), {
                ...
                onError: (errors: any) => {
                    showToast({
                        type: 'error',
                        message: t('common.client_create_error') || 'Failed to create client',
                        duration: 3000,
                    });
                }
            });
                        ...
                        >
                            <DialogPanel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white dark:bg-dark-700 p-6 text-left align-middle shadow-xl transition-all">
                                <DialogTitle className="flex items-center justify-between text-lg font-medium leading-6 text-gray-900 dark:text-dark-100">
                                    <span>
                                        {isEditing ? t('common.edit_client') : t('common.add_client')}
                                    </span>
                                    <button
                                        type="button"
                                        onClick={onClose}
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
                                    {isEditing
                                            ? t('common.edit_client_info', { name: `${client?.first_name} ${client?.last_name}` })
